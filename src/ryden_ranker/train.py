...
def train(data_dir: Path, model_out: Path) -> dict[str, float]:
    ...
    ranker = SklearnPlaceRanker().fit(train_frame)
    valid_frame["score"] = ranker.predict(valid_frame)
    ranker.save(model_out)

    metrics = ranking_metrics(valid_frame, "score")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Ryden sklearn ranking model.")
    parser.add_argument("--data", type=Path, default=Path("data/processed"))
    parser.add_argument("--model-out", type=Path, default=Path("models/ranker.pkl"))
    args = parser.parse_args()

    metrics = train(args.data, args.model_out)
